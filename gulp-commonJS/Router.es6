...
class Router {
	...
	var _every = function(arr, iterator) {
	...
	var _asyncEverySeries = function(arr, iterator, callback) {
		if (!arr.length) {
			return callback();
		}
		var completed = 0;
	...
	var terminator = function(routes, delimiter, start, stop) {
		let last = 0, left = 0, right = 0, start = (start || "(").toString(), stop = (stop || ")").toString(), i;
	...
	insertEx(method, path, route, parent) {
		if (method === "once") {
			method = "on";
	...
	path(path, routesFn) {
		let self = this, length = this.scope.length;
		if (path.source) {
			path = path.source.replace(/\\\//ig, "/");
		}
		path = path.split(new RegExp(this.delimiter));
	...
	dispatch(method, path, callback) {
		let self = this,
			fns = this.traverse(method,
				path.replace(QUERY_SEPARATOR, ""), this.routes, ""),
	...
	traverse(method, path, routes, regexp, filter) {
		let fns = [], current, exact, match, next, that;
	...
	extend(methods) {
		let self = this, len = methods.length, i;
